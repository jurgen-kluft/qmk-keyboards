...
static const leader2_t leader2t1_array[] = {
    ...
    [LA_BRACKET_UP]         = {KC_G, KC_U},
    [LA_BRACKET_DOWN]       = {KC_G, KC_D},
...
static leader4_t const leader4t2_array[] = {
...
static leader_config_t leader_config_t2 = {
    ...
    .leader3_array = NULL,
    .leader3_count = 0,
    .leader4_array = leader4t2_array,
    .leader4_count = ARRAY_SIZE(leader4t2_array),
};
...
void execute_leader_action(uint8_t action, uint8_t mode, uint8_t count, uint8_t* keycodes)
{
    ...
    if (count == 1)
    {
        if (mode == 0)
        {
            switch (action)
            {
                case LA_APPEND_START: send_taps2(KC_HOME, KC_HOME); break;
    ...
    else if (count == 2)
    {
        if (mode == 0)
        {
            switch (action)
            {
                ...
                case LA_BRACKET_UP: send_taps1(C(A(KC_U))); break;
                case LA_BRACKET_DOWN: send_taps1(C(A(KC_D))); break;
